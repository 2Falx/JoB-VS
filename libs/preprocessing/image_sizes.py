...
def Calculate_sizes(root, num_workers=-1, remake=False):
...
def Calculate_sizes_general(root, num_workers=-1, remake=False):
    
    file = os.path.join(root, 'dataset.json' )
    dataset = json.load(open(file, 'r'))
    images = [os.path.join(root, x['label']) for x in dataset['training']]

    file_name = os.path.join(root, 'dataset_stats.json')
    
    print(f'Calculating sizes for {file_name}')
    
    labels = len(dataset['labels'])
    
    features = Parallel(n_jobs=num_workers)(delayed(parallel_sizes)(i, labels) for i in images)
